 ...
 *   YUTIANCLAW_QQBOT_SOURCE=1.2.3
 *   YUTIANCLAW_WECOM_SOURCE=./path/to/plugin.tgz
 *   YUTIANCLAW_DINGTALK_SOURCE=latest
 *   YUTIANCLAW_WEIXIN_SOURCE=1.0.2
 *   YUTIANCLAW_FEISHU_SOURCE=2026.5.7
...
const PRUNE_VERSION = 'v5'
const PLUGIN_BUNDLE_VERSION = 'v2'
...
const BUNDLED_PLUGINS = [
  {
    ...
    id: 'openclaw-qqbot',
    label: 'QQ Bot',
    envVar: 'YUTIANCLAW_QQBOT_SOURCE',
  },
  {
    packageName: '@wecom/wecom-openclaw-plugin',
    id: 'wecom-openclaw-plugin',
    label: '企业微信',
    envVar: 'YUTIANCLAW_WECOM_SOURCE',
  },
  {
    packageName: '@dingtalk-real-ai/dingtalk-connector',
    id: 'dingtalk-connector',
    label: '钉钉',
    envVar: 'YUTIANCLAW_DINGTALK_SOURCE',
  },
  {
    packageName: '@tencent-weixin/openclaw-weixin',
    id: 'openclaw-weixin',
    label: '微信',
    envVar: 'YUTIANCLAW_WEIXIN_SOURCE',
  },
  {
    packageName: '@openclaw/feishu',
    id: 'feishu',
    label: '飞书',
    envVar: 'YUTIANCLAW_FEISHU_SOURCE',
    matchOpenclawVersion: true,
  },
]

// ─── npm 注册表（国内优先，官方兜底）───
const BUNDLED_MCP_PACKAGES = [
  {
    packageName: 'chrome-devtools-mcp',
    entry: path.join('build', 'src', 'bin', 'chrome-devtools-mcp.js'),
    envVar: 'YUTIANCLAW_CHROME_DEVTOOLS_MCP_SOURCE',
  },
]
...
async function getLatestNode22Version() {
  ...
  try {
    ...
    if (fs.existsSync(cacheFile)) {
      ...
      log(`获取 Node.js 版本列表失败，使用过期缓存兜底: ${message}`)
      return pickV22(JSON.parse(fs.readFileSync(cacheFile, 'utf-8')))
    }
    throw err
  }
...
function installOpenclaw(opts, gatewayDir) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: 'yutianclaw-gateway', version: '1.0.0', private: true }, null, 2)
  )
  ...
  patchGatewayStartupPerformance(gatewayDir)
  patchReplyMediaFailureWarning(gatewayDir)
  patchRawChatPerformance(gatewayDir)
...
function resolveNpmPackageSource(packageName, envVar) {
  const explicit = readEnv(envVar)
  if (explicit) {
    log(`Using ${envVar}: ${explicit}`)
    return { source: explicit, stamp: `explicit:${explicit}` }
  }

  for (const reg of NPM_REGISTRIES) {
    try {
      const version = execSync(`npm view "${packageName}" version --registry ${reg}`, {
        encoding: 'utf-8',
        timeout: 30_000,
        stdio: ['pipe', 'pipe', 'pipe'],
      }).trim()
      log(`${packageName} latest version: ${version} (${reg})`)
      return { source: version, stamp: `${packageName}@${version}` }
    } catch {}
  }

  die(`Unable to resolve ${packageName}; set ${envVar}`)
}

function installBundledMcpPackages(opts, gatewayDir) {
  for (const item of BUNDLED_MCP_PACKAGES) {
    const stampPath = path.join(gatewayDir, `.${item.packageName.replace(/[^a-z0-9.-]+/gi, '-')}-stamp`)
    const { source, stamp } = resolveNpmPackageSource(item.packageName, item.envVar)
    const targetStamp = `${opts.platform}-${opts.arch}|${stamp}`
    const existing = fs.existsSync(stampPath) ? fs.readFileSync(stampPath, 'utf-8').trim() : ''
    const entryExists = fs.existsSync(path.join(gatewayDir, 'node_modules', item.packageName, item.entry))

    if (existing === targetStamp && entryExists) {
      log(`${item.packageName} already bundled (${stamp}), skipping`)
      continue
    }
    ...
    for (const reg of NPM_REGISTRIES) {
      try {
        execSync(
          `npm install "${item.packageName}@${source}" --omit=dev --install-links --legacy-peer-deps --ignore-scripts` +
            ` --os=${opts.platform} --cpu=${opts.arch} --registry ${reg}`,
          {
            cwd: gatewayDir,
            stdio: 'inherit',
        ...
        log(`Registry ${reg} failed to install ${item.packageName}: ${err.message}`)
      }
    }

    if (!installed) die(`${item.packageName} install failed`)
    if (!fs.existsSync(path.join(gatewayDir, 'node_modules', item.packageName, item.entry))) {
      die(`${item.packageName} entry not found: ${item.entry}`)
    }

    pruneNodeModules(path.join(gatewayDir, 'node_modules'))
    fs.writeFileSync(stampPath, targetStamp)
    log(`${item.packageName} bundled`)
  }
}
  ...
  function walk(dir, depth, rootPkg) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        const isDecl =
          lower.endsWith('.d.ts') || lower.endsWith('.d.mts') || lower.endsWith('.d.cts')
        const isMap = ext === '.map'
...
function patchWindowsSpawn(gatewayDir, platform) {
  ...
  const entries = listJsFiles(distDir)
  const execFiles = entries.filter((f) => /^exec-.*\.js$/.test(path.basename(f)))
  const cliFiles = entries.filter((f) => {
    const normalized = f.split(path.sep).join('/')
    return (
      /^gateway-cli-.*\.js$/.test(path.basename(f)) ||
      normalized === 'cli/gateway-lifecycle.runtime.js'
    )
  })
  const launcherFiles = fs.existsSync(path.join(openclawDir, 'openclaw.mjs'))
    ? ['openclaw.mjs']
    : []
  ...
  const patchCliRespawn = (s) =>
    s
      ...
      .replace(
        /(spawn\(process\.execPath, args, \{)(\r?\n)(\s*)stdio: "inherit",/,
        (_, a, nl, indent) => `${a}${nl}${indent}windowsHide: true,${nl}${indent}stdio: "inherit",`
      )
      .replace(
        /(spawn\(command, args, \{)(\r?\n)(\s*)stdio: "inherit",/,
        (_, a, nl, indent) => `${a}${nl}${indent}windowsHide: true,${nl}${indent}stdio: "inherit",`
      )
  const isCliRespawnReady = (s) =>
    /spawn\(process\.execPath, args[\s\S]{0,200}windowsHide: true/.test(s) ||
    /spawn\(command, args[\s\S]{0,200}windowsHide: true/.test(s)
  const cliR = mergePatchResults(
    applySpawnPatch(distDir, cliFiles, patchCliRespawn, isCliRespawnReady),
    applySpawnPatch(openclawDir, launcherFiles, patchCliRespawn, isCliRespawnReady)
  )

  if (!execFiles.length || !execR.ready) {
    die('exec spawn 补丁未能应用 —— openclaw 版本可能已更新，请检查正则是否仍匹配')
  }
  if ((!cliFiles.length && !launcherFiles.length) || !cliR.ready) {
...
function patchGatewayStartupPerformance(gatewayDir) {
  const openclawDir = path.join(gatewayDir, 'node_modules', 'openclaw')
  const distDir = path.join(openclawDir, 'dist')
  if (!fs.existsSync(distDir)) {
    die(`openclaw dist missing, cannot apply gateway startup patch: ${distDir}`)
  }

  const startupFiles = listJsFiles(distDir).filter((f) =>
    /^server-startup-post-attach-.*\.js$/.test(path.basename(f))
  )
  const result = applySpawnPatch(
    distDir,
    startupFiles,
    (s) =>
      s.replace(
        /if \(params\.providerAuthPrewarm\?\.enabled !== false\) gatewayLifetimeSidecars\.push\(scheduleProviderAuthStatePrewarm\(\{/,
        'if (params.providerAuthPrewarm?.enabled !== false && !isTruthyEnvValue(process.env.OPENCLAW_SKIP_PROVIDER_AUTH_PREWARM)) gatewayLifetimeSidecars.push(scheduleProviderAuthStatePrewarm({'
      ),
    (s) =>
      /OPENCLAW_SKIP_PROVIDER_AUTH_PREWARM/.test(s) &&
      /scheduleProviderAuthStatePrewarm/.test(s)
  )

  if (!startupFiles.length || !result.ready) {
    die('gateway startup patch failed; openclaw startup module shape may have changed')
  }

  log(`Gateway startup patch applied: providerAuthPrewarm=${result.patched}/${result.ready}`)
}

function patchReplyMediaFailureWarning(gatewayDir) {
  const openclawDir = path.join(gatewayDir, 'node_modules', 'openclaw')
  const distDir = path.join(openclawDir, 'dist')
  if (!fs.existsSync(distDir)) {
    die(`openclaw dist missing, cannot apply reply media warning patch: ${distDir}`)
  }

  const payloadFiles = fs
    .readdirSync(distDir)
    .filter((name) => /^reply-payload-.*\.js$/.test(name))
  const warning =
    '⚠️ 文件发送失败：文件可能超过当前渠道限制，或本地文件无法读取。请尝试压缩、拆分文件，或改用飞书云文档/网盘链接发送。'
  const warningLine = `const REPLY_MEDIA_FAILURE_WARNING = ${JSON.stringify(warning)};`
  const result = applySpawnPatch(
    distDir,
    payloadFiles,
    (s) => s.replace(/const REPLY_MEDIA_FAILURE_WARNING = ".*?Media failed\.";?/u, warningLine),
    (s) => s.includes(warningLine)
  )

  if (!payloadFiles.length || !result.ready) {
    die('reply media warning patch failed; openclaw reply payload module shape may have changed')
  }

  log(`Reply media warning patch applied: ${result.patched}/${result.ready}`)
}

function patchRawChatPerformance(gatewayDir) {
  const openclawDir = path.join(gatewayDir, 'node_modules', 'openclaw')
  const distDir = path.join(openclawDir, 'dist')
  if (!fs.existsSync(distDir)) {
    die(`openclaw dist missing, cannot apply raw chat performance patch: ${distDir}`)
  }

  const selectionFiles = fs
    .readdirSync(distDir)
    .filter((name) => /^selection-.*\.js$/.test(name))
  const piFiles = fs
    .readdirSync(distDir)
    .filter((name) => /^pi-embedded-.*\.js$/.test(name) || /^embedded-agent-.*\.js$/.test(name))
  const providerStreamFiles = fs
    .readdirSync(distDir)
    .filter((name) => /^provider-stream-.*\.js$/.test(name))
  const extraParamFiles = fs
    .readdirSync(distDir)
    .filter((name) => /^extra-params-.*\.js$/.test(name))
  const modelAuthFiles = fs
    .readdirSync(distDir)
    .filter((name) => /^model-auth-.*\.js$/.test(name))

  const selectionResult = applySpawnPatch(
    distDir,
    selectionFiles,
    patchRawModelRunSelection,
    (s) =>
      /isRawModelRun \? \[\] : collectRuntimeChannelCapabilities/.test(s) &&
      /yutianclaw-skip-raw-compaction-wait/.test(s) &&
      !/createSystemPromptOverride/.test(s)
  )
  const piResult = applySpawnPatch(
    distDir,
    piFiles,
    patchCustomOpenAiRuntimeAuth,
    (s) => /skipRuntimeAuth = runtimeModel\.api === "openai-completions"/.test(s)
  )
  const providerStreamResult = applySpawnPatch(
    distDir,
    providerStreamFiles,
    patchCustomOpenAiProviderStream,
    (s) => /skipRuntimePlugin = params\.model\.api === "openai-completions"/.test(s)
  )
  const extraParamsResult = applySpawnPatch(
    distDir,
    extraParamFiles,
    patchCustomOpenAiExtraParams,
    (s) => /skipProviderRuntimeWrapper = model\?\.api === "openai-completions"/.test(s)
  )
  const modelAuthResult = applySpawnPatch(
    distDir,
    modelAuthFiles,
    patchCustomOpenAiConfigKeyAuth,
    (s) => /yutianclaw-fast-config-key/.test(s)
  )

  const required = [
    ['selection', selectionFiles, selectionResult],
    ['pi auth', piFiles, piResult],
    ['provider stream', providerStreamFiles, providerStreamResult],
    ['extra params', extraParamFiles, extraParamsResult],
    ['model auth', modelAuthFiles, modelAuthResult],
  ]
  for (const [label, files, result] of required) {
    if (!files.length || result.ready < 1) {
      die(`raw chat performance patch failed for ${label}; openclaw module shape may have changed`)
    }
  }

  log(
    `Raw chat performance patch applied: selection=${selectionResult.patched}/${selectionResult.ready} ` +
      `pi=${piResult.patched}/${piResult.ready} stream=${providerStreamResult.patched}/${providerStreamResult.ready} ` +
      `extra=${extraParamsResult.patched}/${extraParamsResult.ready} auth=${modelAuthResult.patched}/${modelAuthResult.ready}`
  )
}

function patchRawModelRunSelection(source) {
  let s = source
  s = s.replace(
    'function buildAttemptSystemPrompt(params) {\n\tconst baseSystemPrompt =',
    'function buildAttemptSystemPrompt(params) {\n\tif (params.isRawModelRun) return { baseSystemPrompt: "", systemPrompt: "" };\n\tconst baseSystemPrompt ='
  )
  s = s.replace(
    'const runtimeCapabilities = collectRuntimeChannelCapabilities({',
    'const runtimeCapabilities = isRawModelRun ? [] : collectRuntimeChannelCapabilities({'
  )
  s = s.replace(
    'const reactionGuidance = runtimeChannel && params.config ? resolveChannelReactionGuidance({',
    'const reactionGuidance = isRawModelRun ? void 0 : runtimeChannel && params.config ? resolveChannelReactionGuidance({'
  )
  s = s.replace(
    'const reasoningTagHint = isReasoningTagProvider(params.provider, {',
    'const reasoningTagHint = isRawModelRun ? false : isReasoningTagProvider(params.provider, {'
  )
  s = s.replace(
    'const channelActions = runtimeChannel ? listChannelSupportedActions(buildEmbeddedMessageActionDiscoveryInput({',
    'const channelActions = isRawModelRun ? void 0 : runtimeChannel ? listChannelSupportedActions(buildEmbeddedMessageActionDiscoveryInput({'
  )
  s = s.replace(
    'const messageToolHints = runtimeChannel ? resolveChannelMessageToolHints({',
    'const messageToolHints = isRawModelRun ? void 0 : runtimeChannel ? resolveChannelMessageToolHints({'
  )
  s = s.replace(
    'const defaultModelRef = resolveDefaultModelForAgent({\n\t\t\tcfg: params.config ?? {},\n\t\t\tagentId: sessionAgentId\n\t\t});',
    'const defaultModelRef = isRawModelRun ? { provider: params.provider, model: params.modelId } : resolveDefaultModelForAgent({\n\t\t\tcfg: params.config ?? {},\n\t\t\tagentId: sessionAgentId\n\t\t});'
  )
  s = s.replace(
    'const activeProcessSessions = listActiveProcessSessionReferences({ scopeKey: resolveProcessToolScopeKey({',
    'const activeProcessSessions = isRawModelRun ? [] : listActiveProcessSessionReferences({ scopeKey: resolveProcessToolScopeKey({'
  )
  s = s.replace(
    'const { runtimeInfo, userTimezone, userTime, userTimeFormat } = buildSystemPromptParams({',
    'const { runtimeInfo, userTimezone, userTime, userTimeFormat } = isRawModelRun ? { runtimeInfo: { model: `${params.provider}/${params.modelId}`, defaultModel: defaultModelLabel, channel: runtimeChannel, capabilities: [] }, userTimezone: void 0, userTime: void 0, userTimeFormat: void 0 } : buildSystemPromptParams({'
  )
  s = s.replace(
    'const openClawReferences = await resolveOpenClawReferencePaths({',
    'const openClawReferences = isRawModelRun ? {} : await resolveOpenClawReferencePaths({'
  )
  s = s.replace(
    ...
    'const promptContribution = isRawModelRun ? void 0 : params.runtimePlan?.prompt.resolveSystemPromptContribution(promptContributionContext) ?? resolveProviderSystemPromptContribution({'
  )
  s = s.replace(
    'const providerTextTransforms = resolveProviderTextTransforms({',
    'const providerTextTransforms = isRawModelRun && params.model.api === "openai-completions" && params.model.baseUrl && params.provider !== "openai" ? void 0 : resolveProviderTextTransforms({'
  )
  s = s.replace(
    '\t\t\t\ttry {\n\t\t\t\t\tif (onBlockReplyFlush) await onBlockReplyFlush();',
    '\t\t\t\t// yutianclaw-skip-raw-compaction-wait: raw model runs do not need compaction retry polling.\n\t\t\t\tif (isRawModelRun) {\n\t\t\t\t\tif (onBlockReplyFlush) await onBlockReplyFlush();\n\t\t\t\t} else try {\n\t\t\t\t\tif (onBlockReplyFlush) await onBlockReplyFlush();'
  )
  s = s.replace(
    '\t\t\t\tawait sessionLockController.waitForSessionEvents(activeSession);\n\t\t\t\t__ytTraceAttempt("post-compaction.wait-events");',
    '\t\t\t\tif (!isRawModelRun) await sessionLockController.waitForSessionEvents(activeSession);\n\t\t\t\t__ytTraceAttempt("post-compaction.wait-events");'
  )
  s = s.replace(
...
function patchCustomOpenAiRuntimeAuth(source) {
  return source.replace(
    'const runtimeModel = params.getRuntimeModel();\n\t\tconst preparedAuth = await prepareRuntimeAuthForModel({',
    'const runtimeModel = params.getRuntimeModel();\n\t\tconst skipRuntimeAuth = runtimeModel.api === "openai-completions" && runtimeModel.baseUrl && runtimeModel.provider !== "openai";\n\t\tconst preparedAuth = skipRuntimeAuth ? void 0 : await prepareRuntimeAuthForModel({'
  )
}

function patchCustomOpenAiProviderStream(source) {
  let s = source
  s = s.replace(
    'function registerProviderStreamForModel(params) {\n\tconst streamFn = resolveProviderStreamFn({',
    'function registerProviderStreamForModel(params) {\n\tconst skipRuntimePlugin = params.model.api === "openai-completions" && params.model.baseUrl && params.model.provider !== "openai";\n\tconst streamFn = (skipRuntimePlugin ? void 0 : resolveProviderStreamFn({'
  )
  s = s.replace(
...
function patchCustomOpenAiExtraParams(source) {
  return source.replace(
    'const providerStreamBase = agent.streamFn;\n\tconst pluginWrappedStreamFn = providerRuntimeDeps.wrapProviderStreamFn({',
    'const providerStreamBase = agent.streamFn;\n\tconst skipProviderRuntimeWrapper = model?.api === "openai-completions" && model?.baseUrl && provider !== "openai";\n\tconst pluginWrappedStreamFn = skipProviderRuntimeWrapper ? void 0 : providerRuntimeDeps.wrapProviderStreamFn({'
  )
}

function patchCustomOpenAiConfigKeyAuth(source) {
  return source.replace(
    '\tif (cfg?.auth?.profiles || cfg?.auth?.order) {',
    '\t// yutianclaw-fast-config-key: prefer literal provider API keys for OpenAI-compatible custom providers.\n\tif (!profileId && params.modelApi === "openai-completions") {\n\t\tconst directCustomKey = resolveUsableCustomProviderApiKey({\n\t\t\tcfg,\n\t\t\tprovider\n\t\t});\n\t\tif (directCustomKey && !isNonSecretApiKeyMarker(directCustomKey.apiKey)) return {\n\t\t\tapiKey: directCustomKey.apiKey,\n\t\t\tsource: directCustomKey.source,\n\t\t\tmode: "api-key"\n\t\t};\n\t}\n\tif (cfg?.auth?.profiles || cfg?.auth?.order) {'
  )
}
...
function resolvePluginSource(plugin, gatewayDir) {
  ...
  if (plugin.matchOpenclawVersion) {
    const openclawPkg = path.join(gatewayDir, 'node_modules', 'openclaw', 'package.json')
    const hostVersion = JSON.parse(fs.readFileSync(openclawPkg, 'utf-8')).version
    log(`  ${plugin.label}: 跟随 openclaw 版本 ${hostVersion}`)
    return { depSpec: hostVersion, stamp: `${plugin.packageName}@${hostVersion}` }
  }
  for (const reg of NPM_REGISTRIES) {
    try {
...
function patchBundledPluginMetadata(plugin, pluginDir, openclawDir) {
  if (plugin.id !== 'feishu') return

  const hostPkgPath = path.join(openclawDir, 'package.json')
  const pluginPkgPath = path.join(pluginDir, 'package.json')
  if (!fs.existsSync(hostPkgPath) || !fs.existsSync(pluginPkgPath)) return

  try {
    const hostVersion = JSON.parse(fs.readFileSync(hostPkgPath, 'utf-8')).version
    const pluginPkg = JSON.parse(fs.readFileSync(pluginPkgPath, 'utf-8'))
    const buildVersion = pluginPkg.openclaw?.build?.openclawVersion
    if (!hostVersion || buildVersion !== hostVersion) return

    const install = pluginPkg.openclaw?.install
    if (!install || install.minHostVersion === `>=${hostVersion}`) return

    install.minHostVersion = `>=${hostVersion}`
    fs.writeFileSync(pluginPkgPath, JSON.stringify(pluginPkg, null, 2))
    log(`  ${plugin.label}: patched minHostVersion to >=${hostVersion}`)
  } catch (err) {
    log(`  ${plugin.label}: minHostVersion patch skipped: ${err.message}`)
  }
}
...
async function bundlePlugin(plugin, gatewayDir, opts) {
  ...
  const { depSpec, stamp } = resolvePluginSource(plugin, gatewayDir)
  const bundledStamp = `${stamp}|bundle:${PLUGIN_BUNDLE_VERSION}`

  // 增量检测：stamp 匹配 + manifest 存在 → 跳过
  const stampFile = path.join(pluginDir, '.yutianclaw-stamp.json')
  ...
  patchBundledPluginMetadata(plugin, pluginDir, openclawDir)
  ...
  fs.writeFileSync(
    stampFile,
    JSON.stringify({ stamp: bundledStamp, bundledAt: new Date().toISOString() }, null, 2)
  )
...
async function main() {
  ...
  log('\n-- Step 2c: MCP packages --')
  installBundledMcpPackages(opts, gatewayDir)
