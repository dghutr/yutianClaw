...
const api = {
  ...
  app: {
    ...
    showMainWindow: (route?: string): void => ipcRenderer.send('app:show-main-window', route),
  ...
  account: {
    getState: (): Promise<unknown> => ipcRenderer.invoke('account:get-state'),
    login: (payload: { username: string; password: string }): Promise<unknown> =>
      ipcRenderer.invoke('account:login', payload),
    loginPhone: (payload: { phone: string; code: string }): Promise<unknown> =>
      ipcRenderer.invoke('account:login-phone', payload),
    sendSms: (payload: { phone: string; scene?: 'login' | 'register' }): Promise<unknown> =>
      ipcRenderer.invoke('account:send-sms', payload),
    register: (payload: {
      username: string
      password: string
      phone?: string
      code?: string
    }): Promise<unknown> => ipcRenderer.invoke('account:register', payload),
    logout: (): Promise<unknown> => ipcRenderer.invoke('account:logout'),
    refresh: (): Promise<unknown> => ipcRenderer.invoke('account:refresh'),
    syncKey: (): Promise<unknown> => ipcRenderer.invoke('account:sync-key'),
  },
  ...
  gateway: {
    ...
    onSessionActivityChanged: (callback: (payload: unknown) => void): (() => void) => {
      const listener = (_event: unknown, payload: unknown): void => callback(payload)
      ipcRenderer.on('gateway:session-activity-changed', listener)
      return () => ipcRenderer.removeListener('gateway:session-activity-changed', listener)
    },
  },
  ...
  channel: {
    ...
    resolveFeishuPeers: (
      peers: Array<{ kind?: 'user' | 'chat'; id: string }>
    ): Promise<
      Array<{ id: string; kind: 'user' | 'chat'; displayName?: string; error?: string }>
    > => ipcRenderer.invoke('channel:feishu-resolve-peers', peers),
  ...
  log: {
    readYuTianClaw: (): Promise<string[]> => ipcRenderer.invoke('log:read-yutianclaw'),
  ...
  skill: {
    ...
    importLocal: (kind: 'directory' | 'zip', installDir?: string): Promise<unknown> =>
      ipcRenderer.invoke('skill:import-local', kind, installDir),
