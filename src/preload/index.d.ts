...
declare global {
  ...
  interface SessionActivityChange {
    sessions: Array<{
      key: string
      updatedAt?: number
    }>
  ...
  interface YuTianClawAPI {
    ...
    /** Gateway 管理 */
    account: {
      getState: () => Promise<YutianAccountState>
      login: (payload: { username: string; password: string }) => Promise<YutianAccountState>
      loginPhone: (payload: { phone: string; code: string }) => Promise<YutianAccountState>
      sendSms: (payload: { phone: string; scene?: 'login' | 'register' }) => Promise<{
        success: boolean
      }>
      register: (payload: {
        username: string
      ...
      logout: () => Promise<YutianAccountState>
      refresh: () => Promise<YutianAccountState>
      syncKey: () => Promise<YutianAccountState>
    }
    ...
    gateway: {
      ...
      onSessionActivityChanged: (callback: (payload: SessionActivityChange) => void) => () => void
    }
    ...
    skill: {
      ...
      /** 导入本地 skill 目录或 ZIP 包 */
      importLocal: (kind: 'directory' | 'zip', installDir?: string) => Promise<SkillImportResult>
      /** 读取 SKILL.md 内容 */
