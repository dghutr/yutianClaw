...
import { readConfig, repairExternalMcpConfig } from '../config'
import {
  DEFAULT_PORT,
  DEFAULT_BIND,
...
export class GatewayProcess {
  ...
  private async startInternal(recoveredFromLockConflict: boolean): Promise<GatewayStartResult> {
    this.repairConfigBeforeStart()
    // Sync the latest configured gateway port before startup.
   ...
   */
  startStatusPolling(intervalMs = 15000): void {
  ...
  /**
   * Repair common external MCP config shapes before launching the gateway.
   */
  private repairConfigBeforeStart(): void {
    try {
      const result = repairExternalMcpConfig()
      if (result.changed) {
        log.warn(
          `startup config repair: external MCP keys moved to mcp.servers (migrated=${result.migrated.join(',') || '-'}, skipped=${result.skipped.join(',') || '-'})`
        )
      }
    } catch (err) {
      log.warn(`startup config repair skipped: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
  ...
  private syncPortFromConfig(): void {
    try {
      ...
      // Keep the current port when config cannot be read.
    }
  }
  ...
  private async waitForHealth(
    ...
    while (Date.now() < deadline) {
      ...
      // HTTP 探测
      if (readyFromOutput?.()) {
        log.info(`health check passed by gateway ready log (attempts=${attempts})`)
        return true
      }
...
function isGatewayReadyLog(line: string): boolean {
  return /\[(?:gateway)\]\s+ready\b/i.test(line) || /\bgateway ready\b/i.test(line)
}
