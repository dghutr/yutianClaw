...
import { YUTIANCLAW_API_BASE_URL } from '../../shared/urls'
...
const REMOTE_PRESETS_URL = `${YUTIANCLAW_API_BASE_URL}/ai/provider.json`
  ...
  try {
    ...
    try {
      res = await proxyFetch(REMOTE_PRESETS_URL, {
        ...
        headers: { Accept: 'application/json', 'User-Agent': 'YuTianClaw' },
      })
    } finally {
      clearTimeout(timer)
    }
