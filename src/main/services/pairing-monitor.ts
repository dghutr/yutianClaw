...
import { OPENCLAW_HOME, YUTIANCLAW_HOME } from '../constants'
...
/** 拒绝码 sidecar 文件路径 */
const REJECTED_CODES_PATH = join(YUTIANCLAW_HOME, 'rejected-pairing.json')
...
function writeRejectedCodes(data: Record<string, string[]>): void {
  try {
    if (!existsSync(YUTIANCLAW_HOME)) mkdirSync(YUTIANCLAW_HOME, { recursive: true })
