 ...
 *
 * Windows：
 *   - 在 ~/.yutianclaw/bin\ 创建 openclaw.cmd
 *   - 通过 PowerShell [Environment]::SetEnvironmentVariable 写入用户级 PATH
 *
 * macOS / POSIX：
 *   - 在 ~/.yutianclaw/bin/ 创建 openclaw (bash 脚本)
 *   - 在 ~/.zprofile 和 ~/.bash_profile 写入 PATH 注入块
 *
 * 迁移说明：
 *   旧版使用 %LOCALAPPDATA%\YuTianClaw\bin（Windows）或 ~/.openclaw/bin（macOS），
 *   installCli() 会自动清除旧路径并写入新路径 ~/.yutianclaw/bin（两端统一）。
 */
...
const CLI_MARKER = 'YuTianClaw CLI'

/** POSIX RC 文件注入块起始标记 */
const RC_BLOCK_START = '# >>> yutianclaw-cli >>>'
/** POSIX RC 文件注入块结束标记 */
const RC_BLOCK_END = '# <<< yutianclaw-cli <<<'

// ─── 路径解析 ───

/**
 * CLI bin 目录（两端统一）
 * - Windows: ~/.yutianclaw/bin\
 * - POSIX:   ~/.yutianclaw/bin/
 */
function resolveBinDir(): string {
  return join(YUTIANCLAW_HOME, 'bin')
}
...
function buildWinWrapper(nodeBin: string, gatewayEntry: string): string {
  ...
  return [
    ...
    '  echo Error: YuTianClaw Node runtime not found: %APP_NODE% 1>&2',
    '  exit /b 127',
    ')',
    'if not exist "%APP_ENTRY%" (',
    '  echo Error: YuTianClaw gateway entry not found: %APP_ENTRY% 1>&2',
    '  exit /b 127',
    ')',
...
function buildPosixWrapper(nodeBin: string, gatewayEntry: string): string {
  return [
    ...
    'if [ ! -f "$APP_NODE" ]; then',
    '  echo "Error: YuTianClaw Node runtime not found: $APP_NODE" >&2',
    '  exit 127',
    'fi',
    'if [ ! -f "$APP_ENTRY" ]; then',
    '  echo "Error: YuTianClaw gateway entry not found: $APP_ENTRY" >&2',
    '  exit 127',
    'fi',
...
/** 检查路径是否是 YuTianClaw 创建的 wrapper（通过 marker 行识别） */
...
export function posixBuildRcBlock(binDir: string): string {
  return [
    RC_BLOCK_START,
    '# Added by YuTianClaw — https://yutianclaw.cn',
...
/** 从 RC 文件内容中删除 YuTianClaw 注入块，返回修改后的内容（可测试，供单元测试直接导入） */
...
/** 检查是否有任意 RC 文件包含 YuTianClaw 注入块 */
...
export interface CliStatus {
  ...
  wrapperPath: string
  /** openclaw wrapper 文件是否存在（且是 YuTianClaw 创建的） */
  wrapperExists: boolean
  /** clawhub wrapper 文件路径 */
  clawhubWrapperPath: string
  /** clawhub wrapper 文件是否存在（且是 YuTianClaw 创建的） */
 ...
 *
 * 安全操作：只删除 YuTianClaw 自身创建的 wrapper 文件（通过 marker 识别）。
 */
export function uninstallCli(): void {
  ...
  // 仅删除 YuTianClaw 创建的 wrapper（防止误删用户自定义文件）
  for (const wp of [wrapperPath, clawhubWrapperPath]) {
    if (isOurWrapper(wp)) {
      ...
      log.warn(`wrapper 文件不是 YuTianClaw 创建的，跳过删除: ${wp}`)
    }
  }
