...
const repairExternalMcpConfigMock = vi.fn()
...
vi.mock('../../main/config', () => ({
  ...
  repairExternalMcpConfig: repairExternalMcpConfigMock,
}))
...
describe('GatewayProcess', () => {
  beforeEach(() => {
    ...
    resolveGatewayTokenMock.mockReturnValue('yutianclaw-token')
    readConfigMock.mockReturnValue({})
    repairExternalMcpConfigMock.mockReturnValue({
      changed: false,
      migrated: [],
      skipped: [],
      removedLegacyKeys: [],
  ...
  it('通过 /ready 后进入 running', async () => {
    ...
    expect(repairExternalMcpConfigMock).toHaveBeenCalledTimes(1)
