...
describe('posixBuildRcBlock', () => {
  it('包含开始标记', () => {
    expect(posixBuildRcBlock(BIN_DIR)).toContain('# >>> yutianclaw-cli >>>')
  })
...
describe('posixStripRcBlock', () => {
  ...
  it('块在文件末尾时，删除块后前面的内容保留', () => {
    ...
    expect(result).not.toContain('yutianclaw-cli')
  })
  ...
  it('删除后不含开始标记', () => {
    ...
    expect(posixStripRcBlock(content)).not.toContain('# >>> yutianclaw-cli >>>')
  })
