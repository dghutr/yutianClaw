...
function findAccountBinding(
  bindings: ChannelCardProps['bindings'],
  channelKey: string,
  accountId: string
): ChannelCardProps['bindings'][number] | undefined {
  const exact = bindings.find(
    (binding) => binding.match?.channel === channelKey && binding.match?.accountId === accountId
  )
        ...
        {!hasAccounts ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {accountIds.map((accountId) => {
              ...
              const currentBinding = findAccountBinding(bindings, channelKey, accountId)
