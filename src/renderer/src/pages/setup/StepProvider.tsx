...
function getDefaultModelId(platform?: { models?: Array<{ id: string }> }): string {
...
function StepProvider({ data, updateData }: Props): React.ReactElement {
  ...
  const selectPlatform = (platformKey: string) => {
    const platform = selectedPreset?.platforms.find((item) => item.key === platformKey)
    updateData({ platformKey, apiKey: '', modelId: getDefaultModelId(platform) })
    setApiKey('')
    setVerifyState('idle')
    setVerifyMessage('')
  }
                ...
                label: (
                  <Flex justify="space-between" gap={10} align="start">
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <Tooltip
                        title={formatModelDisplayName(selectedPlatform.key, model.id, model.name)}
                      >
                        <div
                          style={{
                            fontWeight: 500,
                        ...
                        >
                          {formatModelDisplayName(selectedPlatform.key, model.id, model.name)}
                        </div>
                      </Tooltip>
                      {!isYutianProviderKey(selectedPlatform.key) && (
                        <Text
                          type="secondary"
                          style={{
                            fontSize: 12,
  ...
  return (
    <div style={{ maxWidth: 1120, width: '100%' }}>
      ...
      <Alert
        type="info"
        showIcon
        message={t('setup.provider.optionalTitle')}
        description={t('setup.provider.optionalDesc')}
