...
export function ProviderSetupDrawer({
  ...
  const effectivePlatform = presetBrand
    ...
    : null
  const yutianPlatform = Boolean(effectivePlatform && isYutianProviderKey(effectivePlatform.key))
      ...
      {effectivePlatform && !isEdit && effectivePlatform.models.length > 0 && (
        <div>
          ...
          {!yutianPlatform && (
            <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
              ...
              <Button
                size="small"
                onClick={() =>
                  setSelectedModelIds(getRecommendedModelIds(effectivePlatform.models))
                }
              >
          ...
          {effectivePlatform.models.map((m) => {
            ...
            const yutianModel = isYutianProviderKey(effectivePlatform.key)
            const displayName = formatModelDisplayName(effectivePlatform.key, m.id, m.name)
            return (
              <div
                key={m.id}
                style={{
              ...
              >
                ...
                <span
                  style={{
                    fontFamily: yutianModel ? 'inherit' : 'monospace',
                    fontSize: 12,
                    color: '#555',
                    flex: 1,
                    fontWeight: yutianModel ? 600 : 400,
                  }}
                >
                  {displayName}
                </span>
