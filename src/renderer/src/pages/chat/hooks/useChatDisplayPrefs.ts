...
const STORAGE_KEY = 'yutianclaw.chat.display.v2'

function loadInitialPrefs(): ChatDisplayPrefsState {
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY)
    if (!raw) return { showThinking: true, showToolCalls: true, showUsage: true }
    ...
    return { showThinking: true, showToolCalls: true, showUsage: true }
  }
}
