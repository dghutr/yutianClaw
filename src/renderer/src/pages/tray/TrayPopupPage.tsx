...
const WEBSITE_URL = 'https://claw.yutianedu.com'
...
export default function TrayPopupPage(): React.ReactElement {
  ...
  const writeTrayActionLog = (action: string): void => {
    window.api.log
      .write({
        level: 'info',
        tag: 'tray-popup',
        message: `tray action: ${action}`,
      })
      .catch(() => undefined)
  }
  ...
  const handleStart = (): void => {
    writeTrayActionLog('start-gateway')
    setLoading('start')
    window.api.gateway.startWithRecovery().catch(() => setLoading(null))
  }
  const handleStop = (): void => {
    writeTrayActionLog('stop-gateway')
    setLoading('stop')
    window.api.gateway.stop().catch(() => setLoading(null))
  }
  const handleRestart = (): void => {
    writeTrayActionLog('restart-gateway')
    setLoading('restart')
    window.api.gateway.restart().catch(() => setLoading(null))
  }

  const handleOpenWebUI = async (): Promise<void> => {
    writeTrayActionLog('open-openclaw-web-ui')
    if (!isRunning) return
  ...
  const handleOpenMainWindow = (): void => {
    writeTrayActionLog('open-main-window')
    window.api.app.showMainWindow('/dashboard')
  }
  ...
  return (
    ...
    >
      ...
      {/* ── 打开主窗口 ── */}
      <MenuItem onClick={handleOpenMainWindow}>
        <span
