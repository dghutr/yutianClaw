...
- **产品名称：** YuTianClaw（Click + Claw — "点击即装 OpenClaw"）
- **包名 / App ID：** `cn.yutianclaw.app`
- **域名：** yutianclaw.cn / yutianclaw.cloud 
- **版本策略：** YuTianClaw 应用本体使用 semver `x.y.z`；内置 OpenClaw 版本继续沿用其上游日历版本
- **目标平台：** macOS（arm64 + x64）、Windows（x64 + arm64）

## 产品定位

YuTianClaw 是面向**小白用户**的 OpenClaw 桌面管理工具，内置 Node.js 22 + openclaw，零依赖安装即用。
